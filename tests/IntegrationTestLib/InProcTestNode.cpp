...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const Fortress::Currency& currency) : 
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new Fortress::core(m_currency, NULL, log));
    protocol.reset(new Fortress::FortressProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new Fortress::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_Fortress_protocol(protocol.get());

    Fortress::NetNodeConfig p2pConfig;
  ...
  core->deinit();
  p2pNode->deinit();
  core->set_Fortress_protocol(NULL);
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, Fortress::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<Fortress::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new Fortress::InProcessNode(*core, *protocol));
