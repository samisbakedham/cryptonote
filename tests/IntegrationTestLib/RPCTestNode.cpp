...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, Fortress::Block& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<Fortress::INode>& node) {
  std::unique_ptr<Fortress::INode> newNode(new Fortress::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    ...
    Fortress::COMMAND_RPC_GET_INFO::response rsp;
