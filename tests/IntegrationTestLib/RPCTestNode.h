...
namespace Tests {
  ...
  public:
    ...
    virtual bool getBlockTemplate(const std::string& minerAddress, Fortress::Block& blockTemplate, uint64_t& difficulty) override;
    virtual bool submitBlock(const std::string& block) override;
    virtual bool getTailBlockId(Crypto::Hash& tailBlockId) override;
    virtual bool makeINode(std::unique_ptr<Fortress::INode>& node) override;
  ...
  private:
    ...
    Fortress::HttpClient m_httpClient;
  };
}
