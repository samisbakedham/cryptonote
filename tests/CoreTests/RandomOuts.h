...
private:
  ...
  bool checkHalfUnlocked(Fortress::core& c, size_t ev_index, const std::vector<test_event_entry>& events);
  bool checkFullyUnlocked(Fortress::core& c, size_t ev_index, const std::vector<test_event_entry>& events);

  bool request(Fortress::core& c, uint64_t amount, size_t mixin, Fortress::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp);
