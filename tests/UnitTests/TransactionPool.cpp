...
#include "FortressCore/TransactionPool.h"
...
class TransactionValidator : public Fortress::ITransactionValidator {
  virtual bool checkTransactionInputs(const Fortress::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const Fortress::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const Fortress::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const Fortress::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const Fortress::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const Fortress::Currency& currency, Logging::ILogger& logger) :
  ...
  public:
    ...
    Fortress::RealTimeProvider m_time;
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = Fortress::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }
