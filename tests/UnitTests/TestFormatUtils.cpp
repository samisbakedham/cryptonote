...
TEST(parseTransactionExtra, handles_empty_extra)
{
  ...
  std::vector<Fortress::TransactionExtraField> tx_extra_fields;
  ASSERT_TRUE(Fortress::parseTransactionExtra(extra, tx_extra_fields));
...
TEST(parseTransactionExtra, handles_padding_only_size_1)
{
  ...
  ASSERT_EQ(typeid(Fortress::TransactionExtraPadding), tx_extra_fields[0].type());
  ASSERT_EQ(1, boost::get<Fortress::TransactionExtraPadding>(tx_extra_fields[0]).size);
}
...
TEST(parseTransactionExtra, handles_padding_only_size_2)
{
  ...
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(Fortress::TransactionExtraPadding), tx_extra_fields[0].type());
  ASSERT_EQ(2, boost::get<Fortress::TransactionExtraPadding>(tx_extra_fields[0]).size);
}
...
TEST(parseTransactionExtra, handles_padding_only_max_size)
{
  ...
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(Fortress::TransactionExtraPadding), tx_extra_fields[0].type());
  ASSERT_EQ(TX_EXTRA_NONCE_MAX_COUNT, boost::get<Fortress::TransactionExtraPadding>(tx_extra_fields[0]).size);
}
...
TEST(parseTransactionExtra, handles_padding_only_exceed_max_size)
{
  ...
  std::vector<Fortress::TransactionExtraField> tx_extra_fields;
  ASSERT_FALSE(Fortress::parseTransactionExtra(extra, tx_extra_fields));
}
...
TEST(parseTransactionExtra, handles_pub_key_only)
{
  ...
  ASSERT_TRUE(Fortress::parseTransactionExtra(extra, tx_extra_fields));
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(Fortress::TransactionExtraPublicKey), tx_extra_fields[0].type());
}
...
TEST(parseTransactionExtra, handles_extra_nonce_only)
{
  ...
  ASSERT_TRUE(Fortress::parseTransactionExtra(extra, tx_extra_fields));
  ASSERT_EQ(1, tx_extra_fields.size());
  ASSERT_EQ(typeid(Fortress::TransactionExtraNonce), tx_extra_fields[0].type());
  Fortress::TransactionExtraNonce extra_nonce = boost::get<Fortress::TransactionExtraNonce>(tx_extra_fields[0]);
...
TEST(parseTransactionExtra, handles_pub_key_and_padding)
{
  ...
  ASSERT_EQ(typeid(Fortress::TransactionExtraPublicKey), tx_extra_fields[0].type());
  ASSERT_EQ(typeid(Fortress::TransactionExtraPadding), tx_extra_fields[1].type());
}
...
TEST(parse_and_validate_tx_extra, is_valid_tx_extra_parsed)
{
  ...
  Fortress::Transaction tx = AUTO_VAL_INIT(tx);
  Fortress::AccountBase acc;
  acc.generate();
  Fortress::BinaryArray b = Common::asBinaryArray("dsdsdfsdfsf");
  ASSERT_TRUE(currency.constructMinerTx(0, 0, 10000000000000, 1000, currency.minimumFee(), acc.getAccountKeys().address, tx, b, 1));
  Crypto::PublicKey tx_pub_key = Fortress::getTransactionPublicKeyFromExtra(tx.extra);
  ASSERT_NE(tx_pub_key, Fortress::NULL_PUBLIC_KEY);
}
TEST(parse_and_validate_tx_extra, fails_on_big_extra_nonce)
{
  ...
  Fortress::AccountBase acc;
  acc.generate();
  Fortress::BinaryArray b(TX_EXTRA_NONCE_MAX_COUNT + 1, 0);
...
TEST(parse_and_validate_tx_extra, fails_on_wrong_size_in_extra_nonce)
{
  ...
  tx.extra[1] = 255;
  std::vector<Fortress::TransactionExtraField> tx_extra_fields;
  ASSERT_FALSE(Fortress::parseTransactionExtra(tx.extra, tx_extra_fields));
}
TEST(validate_parse_amount_case, validate_parse_amount)
{
  Logging::LoggerGroup logger;
  Fortress::Currency currency = Fortress::CurrencyBuilder(logger).numberOfDecimalPlaces(8).currency();
  uint64_t res = 0;
