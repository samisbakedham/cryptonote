...
namespace Fortress {
  ...
  public:
    ...
    TransactionImpl(const Fortress::Transaction& tx);
  ...
  std::unique_ptr<ITransaction> createTransaction(const Fortress::Transaction& tx) {
    return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
  }

  TransactionImpl::TransactionImpl() {   
    Fortress::KeyPair txKeys(Fortress::generateKeyPair());
  ...
  TransactionImpl::TransactionImpl(const Fortress::Transaction& tx) : transaction(tx) {
    extra.parse(transaction.extra);
  }
  ...
  bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress& addr, const SecretKey& viewSecretKey, std::vector<uint32_t>& out, uint64_t& amount) const {
    return ::Fortress::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
  }
