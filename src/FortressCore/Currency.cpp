...
bool Currency::parseAccountAddressString(const std::string& str, AccountPublicAddress& addr) const {
  uint64_t prefix;
  if (!Fortress::parseAccountAddressString(prefix, addr, str)) {
    return false;
  }
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  maxBlockNumber(parameters::Fortress_MAX_BLOCK_NUMBER);
  maxBlockBlobSize(parameters::Fortress_MAX_BLOCK_BLOB_SIZE);
  maxTxSize(parameters::Fortress_MAX_TX_SIZE);
  publicAddressBase58Prefix(parameters::Fortress_PUBLIC_ADDRESS_BASE58_PREFIX);
  minedMoneyUnlockWindow(parameters::Fortress_MINED_MONEY_UNLOCK_WINDOW);

  timestampCheckWindow(parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
  blockFutureTimeLimit(parameters::Fortress_BLOCK_FUTURE_TIME_LIMIT);

  moneySupply(parameters::MONEY_SUPPLY);
  emissionSpeedFactor(parameters::EMISSION_SPEED_FACTOR);

  rewardBlocksWindow(parameters::Fortress_REWARD_BLOCKS_WINDOW);
  blockGrantedFullRewardZone(parameters::Fortress_BLOCK_GRANTED_FULL_REWARD_ZONE);
  minerTxBlobReservedSize(parameters::Fortress_COINBASE_BLOB_RESERVED_SIZE);

  numberOfDecimalPlaces(parameters::Fortress_DISPLAY_DECIMAL_POINT);
  ...
  lockedTxAllowedDeltaSeconds(parameters::Fortress_LOCKED_TX_ALLOWED_DELTA_SECONDS);
  lockedTxAllowedDeltaBlocks(parameters::Fortress_LOCKED_TX_ALLOWED_DELTA_BLOCKS);

  mempoolTxLiveTime(parameters::Fortress_MEMPOOL_TX_LIVETIME);
  mempoolTxFromAltBlockLiveTime(parameters::Fortress_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME);
  numberOfPeriodsToForgetTxDeletedFromPool(parameters::Fortress_NUMBER_OF_PERIODS_TO_FORGET_TX_DELETED_FROM_POOL);

  fusionTxMaxSize(parameters::FUSION_TX_MAX_SIZE);
  fusionTxMinInputCount(parameters::FUSION_TX_MIN_INPUT_COUNT);
  fusionTxMinInOutCountRatio(parameters::FUSION_TX_MIN_IN_OUT_COUNT_RATIO);

  blocksFileName(parameters::Fortress_BLOCKS_FILENAME);
  blocksCacheFileName(parameters::Fortress_BLOCKSCACHE_FILENAME);
  blockIndexesFileName(parameters::Fortress_BLOCKINDEXES_FILENAME);
  txPoolFileName(parameters::Fortress_POOLDATA_FILENAME);
  blockchinIndicesFileName(parameters::Fortress_BLOCKCHAIN_INDICES_FILENAME);
...
Transaction CurrencyBuilder::generateGenesisTransaction() {
  ...
  Fortress::AccountPublicAddress ac = boost::value_initialized<Fortress::AccountPublicAddress>();
