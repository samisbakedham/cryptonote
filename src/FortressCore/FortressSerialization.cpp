...
#include "FortressSerialization.h"
...
struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const Fortress::BaseInput) { return  0xff; }
  uint8_t operator()(const Fortress::KeyInput) { return  0x2; }
  uint8_t operator()(const Fortress::MultisignatureInput) { return  0x3; }
  uint8_t operator()(const Fortress::KeyOutput) { return  0x2; }
  uint8_t operator()(const Fortress::MultisignatureOutput) { return  0x3; }
  uint8_t operator()(const Fortress::Transaction) { return  0xcc; }
  uint8_t operator()(const Fortress::Block) { return  0xbb; }
};

struct VariantSerializer : boost::static_visitor<> {
  VariantSerializer(Fortress::ISerializer& serializer, const std::string& name) : s(serializer), name(name) {}

  template <typename T>
  void operator() (T& param) { s(param, name); }

  Fortress::ISerializer& s;
  std::string name;
};

void getVariantValue(Fortress::ISerializer& serializer, uint8_t tag, Fortress::TransactionInput& in) {
  switch(tag) {
  case 0xff: {
    Fortress::BaseInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  ...
  case 0x3: {
    Fortress::MultisignatureInput v;
    serializer(v, "value");
    in = v;
    break;
  }
...
void getVariantValue(Fortress::ISerializer& serializer, uint8_t tag, Fortress::TransactionOutputTarget& out) {
  switch(tag) {
  case 0x2: {
    Fortress::KeyOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
  case 0x3: {
    Fortress::MultisignatureOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
...
template <typename T>
bool serializePod(T& v, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializer.binary(&v, sizeof(v), name);
}

bool serializeVarintVector(std::vector<uint32_t>& vector, Fortress::ISerializer& serializer, Common::StringView name) {
...
bool serialize(PublicKey& pubKey, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(pubKey, name, serializer);
}

bool serialize(SecretKey& secKey, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(secKey, name, serializer);
}

bool serialize(Hash& h, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(h, name, serializer);
}

bool serialize(KeyImage& keyImage, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(keyImage, name, serializer);
}

bool serialize(chacha8_iv& chacha, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(chacha, name, serializer);
}

bool serialize(Signature& sig, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(sig, name, serializer);
}

bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name, Fortress::ISerializer& serializer) {
  return serializePod(ecScalar, name, serializer);
}

bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name, Fortress::ISerializer& serializer) {
