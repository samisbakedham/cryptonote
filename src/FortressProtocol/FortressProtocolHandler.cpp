...
#include "FortressProtocolHandler.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const FortressConnectionContext& context) {
...
FortressProtocolHandler::FortressProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t FortressProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void FortressProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void FortressProtocolHandler::onConnectionOpened(FortressConnectionContext& context) {
}

void FortressProtocolHandler::onConnectionClosed(FortressConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IFortressProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != FortressConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IFortressProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void FortressProtocolHandler::stop() {
  m_stop = true;
}

bool FortressProtocolHandler::start_sync(FortressConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == FortressConnectionContext::state_synchronizing) {
...
bool FortressProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void FortressProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const FortressConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t FortressProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool FortressProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, FortressConnectionContext& context, bool is_inital) {
  if (context.m_state == FortressConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == FortressConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = FortressConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = FortressConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = FortressConnectionContext::state_sync_required;
  }
...
bool FortressProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, FortressConnectionContext& ctx, Handler handler) {
...
int FortressProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, FortressConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &FortressProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &FortressProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &FortressProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &FortressProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &FortressProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &FortressProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &FortressProtocolHandler::handleRequestTxPool)
...
int FortressProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, FortressConnectionContext& context) {
  ...
  if (context.m_state != FortressConnectionContext::state_normal) {
    return 1;
  }
  ...
  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = FortressConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = FortressConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int FortressProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, FortressConnectionContext& context) {
...
int FortressProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, FortressConnectionContext& context) {
...
int FortressProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, FortressConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = FortressConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == FortressConnectionContext::state_synchronizing) {
...
int FortressProtocolHandler::processObjects(FortressConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool FortressProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int FortressProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, FortressConnectionContext& context) {
...
bool FortressProtocolHandler::request_missing_objects(FortressConnectionContext& context, bool check_having_blocks) {
...
bool FortressProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IFortressProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int FortressProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, FortressConnectionContext& context) {
...
int FortressProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     FortressConnectionContext& context) {
...
void FortressProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void FortressProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void FortressProtocolHandler::requestMissingPoolTransactions(const FortressConnectionContext& context) {
...
void FortressProtocolHandler::updateObservedHeight(uint32_t peerHeight, const FortressConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void FortressProtocolHandler::recalculateMaxObservedHeight(const FortressConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const FortressConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t FortressProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool FortressProtocolHandler::addObserver(IFortressProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool FortressProtocolHandler::removeObserver(IFortressProtocolObserver* observer) {
