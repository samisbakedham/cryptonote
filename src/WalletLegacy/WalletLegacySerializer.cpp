...
WalletLegacySerializer::WalletLegacySerializer(Fortress::AccountBase& account, WalletUserTransactionsCache& transactionsCache) :
  account(account),
...
void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& cache) {
  ...
  Fortress::BinaryOutputStreamSerializer serializer(plainStream);
  saveKeys(serializer);
...
void WalletLegacySerializer::saveKeys(Fortress::ISerializer& serializer) {
  Fortress::KeysStorage keys;
  Fortress::AccountKeys acc = account.getAccountKeys();
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& cache) {
  ...
  Fortress::BinaryInputStreamSerializer serializerEncrypted(stdStream);
...
void WalletLegacySerializer::loadKeys(Fortress::ISerializer& serializer) {
  Fortress::KeysStorage keys;

  keys.serialize(serializer, "keys");

  Fortress::AccountKeys acc;
