...
protected:
  ...
  struct ReceiverAmounts {
    Fortress::AccountPublicAddress receiver;
  ...
  void updateBalance(Fortress::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(Fortress::ITransfersContainer* container) const;

  Fortress::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const Fortress::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<Fortress::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const Fortress::Transaction& FortressTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const Fortress::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, Fortress::ITransfersContainer* container);
  ...
  Fortress::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
